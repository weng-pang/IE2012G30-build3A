...
//echo $this->Form->button('Back',array('onclick'=>"location.href='".$this->Html->url($urlView)."'"));
	...
	if ($this->Session->read('Auth.User.role') == 'staff' || $this->Session->read('Auth.User.role') == 'superstaff' || $this->Session->read('Auth.User.role') == 'admin'){
		$isStaff = true;
	} else {
		$isStaff = false;
	}
...
?>                                                                                                                            
	...
	<dl>
		...
		<dd>
			<?php echo h($job['Task']['id']); ?>
			&nbsp;
		</dd>
                <dt><?php echo __('Order ID'); ?></dt>
		<dd>
			<?php echo h($job['Task']['order_id']); ?>
			&nbsp;
		</dd>
		...
		<dd>
			<?php echo h($job['Task']['client_name']); ?>
			&nbsp;
		</dd>
                <dt><?php echo __('User Name'); ?></dt>
		<dd>
			<?php echo h($job['Task']['user_name']); ?>
			&nbsp;
		</dd>
                <dt><?php echo __('Type'); ?></dt>
		<dd>
			<?php echo h($job['Task']['type']); ?>
			&nbsp;
		</dd>
        ...
        <dd>
			...
			// for customer only - only viewing capability
				echo h($staff['Staff']['name']);
			}?>
        </dd>
        <dt><?php echo __('Created'); ?></dt>
		<dd>
			<?php echo h($job['Task']['created']); ?>
			&nbsp;
		</dd>
		<dt><?php echo __('Due'); ?></dt>
		<dd>
			<?php echo h($job['Task']['due_date']); ?>
			&nbsp;
		</dd>
		<dt><?php echo __('Closed Date'); ?></dt>
		<dd>
			<?php echo h($job['Task']['closed_date']); ?>
			&nbsp;
		<dt><?php echo __('Priority'); ?></dt>
		<dd><?php
			if ($isStaff){
			// for Staff only
				echo $this->Form->input('job_priority',array('label'=>false,'div'=>false,'default'=>$job['Task']['job_priority'],'hiddenField' => false));
			} else{
			// for customer only - only viewing capability
...
<div class="related">
    ...
    <?php 
    	...
    	<?php foreach($jobNotes as $jobNote){ ?>
	    	<tr>
            	<td>Status:<strong> <?php echo $jobNote['JobStatus']['job_status']; ?></strong> By:<strong><?php echo $this -> Session -> read('Auth.User.username');?></strong></td>
            	<td>Added:<strong> <?php echo $jobNote['JobNote']['created']; ?></strong></td>
            </tr>
    	    <tr>
            	<td>Customer Note</td>
