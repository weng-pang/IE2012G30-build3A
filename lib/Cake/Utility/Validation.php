 ...
 */
	public static function luhn($check, $deep = true) {
		if (is_array($check)) {
			extract(self::_defaults($check));
		}
		if ($deep !== true) {
	...
	/*public static function luhn($string, $deep = true){
		// Uncomment echos for Verbose mode
		if ($deep !== true) {
			return true;
		}
		// obtain string size
		...
		if ($count%2 == 0){
			// even position
			$presum = (int)substr($string,$i,1);
			//echo 'presum: '.$presum.'<br>';
			// each number at even position is multiplied
			$presum = $presum * 2;
			
			if ($presum >= 10){
				// presum larger than 10
				// convert to string, then extract each and sum it up.
				$presum = ((int)substr(strval($presum),0,1))+((int)substr(strval($presum),1,1));
			}
				$sum += $presum;
				//echo 'Even<br>';
			}else{
				// odd position
				//echo 'Odd presum: '.substr($string,$i,1).'<br>';
				$sum += (int)substr($string,$i,1);
				//echo 'Odd<br>';
			}
			$count++;
			//echo $sum.'<br>';
		}
		// add the last checksum
		$sum += (int)substr($string,$size-1,1);
		//echo $sum.'<br>';
		return ($sum%10==0);
	}
*/
